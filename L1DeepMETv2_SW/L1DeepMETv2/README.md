...
<!-- 2. When you run the training script right after step 1, the training script will fetch the `.npz` files from `./data_ttbar/raw` and produce dataloader from these files, using the functions defined in [`./model/data_loader.py`](https://github.com/DeepMETv2/L1DeepMETv2/blob/master/model/data_loader.py). This dataloader-producing takes some time,  -->
You can use the dataloader already produced and saved in `processed.tar.gz` file under `data_delphes` directory. Uncompress this file and place the `.pt` files under suitable directory for use.
